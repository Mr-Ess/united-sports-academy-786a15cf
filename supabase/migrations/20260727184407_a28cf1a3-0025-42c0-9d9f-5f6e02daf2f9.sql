...
-- ========= RLS policies =========
-- branches: everyone authenticated can read; only super_admin/top_management can write
...
-- academy_profiles: user manages own; super_admin manages all
...
-- academy_user_roles: read own or super_admin; only super_admin writes
...
-- page_permissions: readable by all; only super_admin writes
...
-- audit_log: super_admin + top_management read; anyone authenticated can insert
...
-- ========= Seed super_admin for the.one.behind.kemetrise@gmail.com =========
DO $$
DECLARE u_id uuid;
BEGIN
  SELECT id INTO u_id FROM auth.users WHERE email = 'the.one.behind.kemetrise@gmail.com' LIMIT 1;
  IF u_id IS NOT NULL THEN
    INSERT INTO public.academy_user_roles (user_id, role, branch_id)
    VALUES (u_id, 'super_admin', NULL)
    ON CONFLICT DO NOTHING;
    INSERT INTO public.academy_profiles (user_id, default_branch_id, language)
    VALUES (u_id, '11111111-1111-1111-1111-111111111111', 'ar')
    ON CONFLICT (user_id) DO UPDATE SET default_branch_id = EXCLUDED.default_branch_id;
  END IF;
END $$;

-- ========= Seed page_permissions for all academy paths =========
